...
export function recordToolMetric(tool: string, latencyMs: number, isError: boolean, transport: TransportMode): void {
  ...
  incrementCounter("spctre.mcp.tool.calls", 1, { tool, transport, outcome: isError ? "error" : "success" });
  if (isError) incrementCounter("spctre.mcp.tool.errors", 1, { tool, transport });
  recordDuration("spctre.mcp.tool.duration", latencyMs, { tool, transport, outcome: isError ? "error" : "success" });
  ...
  if (p95 > SLO_P95_MS) {
    logger.warn("MCP SLO violation", { event: "mcp.slo_violation", threshold: "p95", tool, transport, value_ms: p95, limit_ms: SLO_P95_MS });
  }
  if (p99 > SLO_P99_MS) {
    logger.warn("MCP SLO violation", { event: "mcp.slo_violation", threshold: "p99", tool, transport, value_ms: p99, limit_ms: SLO_P99_MS });
  }
  if (errorRate > SLO_ERROR_RATE_THRESHOLD) {
    logger.warn("MCP SLO violation", { event: "mcp.slo_violation", threshold: "error_rate", tool, transport, value: errorRate, limit: SLO_ERROR_RATE_THRESHOLD });
  }
}
...
export function buildToolMetricsSnapshot(): Record<string, unknown> {
  ...
  for (const [tool, w] of toolWindows) {
    ...
    metrics[tool] = {
      ...
      slo: { p95_limit_ms: SLO_P95_MS, p99_limit_ms: SLO_P99_MS, error_rate_limit: SLO_ERROR_RATE_THRESHOLD },
    };
  }
  return metrics;
}
