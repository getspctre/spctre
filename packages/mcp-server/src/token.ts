...
export class AccessTokenManager {
  ...
  async getValidAccessToken(): Promise<string> {
    ...
    if (!this.refreshToken) {
      ...
      throw new TokenLifecycleError(
        "no_refresh_token",
        "No valid access token and no refresh token configured.",
      );
    }
  ...
  private applyRefreshResponse(data: TokenRefreshResponse, attempt: number): void {
    ...
    emitTokenEvent("token.refresh_ok", {
      attempt,
      expires_at: new Date(this.expiresAtMs).toISOString(),
    });
  ...
  private throwPermanentRefreshFailure(err: unknown, status: number, attempt: number): never {
    ...
    const isRevoked = String(body?.error ?? "")
      .toLowerCase()
      .includes("revoked");
    ...
    throw new TokenLifecycleError(
      kind,
      `Token refresh rejected by server (${status}): ${body?.error ?? errorMessage(err)}`,
    );
  }
  ...
  private async refreshWithRetry(): Promise<void> {
    if (!this.refreshToken) {
      ...
      throw new TokenLifecycleError(
        "no_refresh_token",
        "Cannot refresh: refresh token not configured.",
      );
    }
    ...
    for (let attempt = 1; attempt <= REFRESH_MAX_ATTEMPTS; attempt++) {
      try {
        const response: import("axios").AxiosResponse<TokenRefreshResponse> =
        ...
        emitTokenEvent("token.refresh_retrying", {
          attempt,
    ...
    emitTokenEvent("token.refresh_exhausted", {
      attempts: REFRESH_MAX_ATTEMPTS,
      error: errorMessage(lastErr),
    });
    incrementCounter("spctre.token.refresh", 1, {
      service: "mcp",
      outcome: "refresh_network_error",
    });
    throw new TokenLifecycleError(
      "refresh_network_error",
      `Token refresh failed after ${REFRESH_MAX_ATTEMPTS} attempts: ${errorMessage(lastErr)}`,
    );
  }
