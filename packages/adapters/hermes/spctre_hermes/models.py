...
class EvaluationResult(BaseModel):
    ...
    def from_response(cls, payload: Dict[str, Any]) -> "EvaluationResult":
        ...
        policy_refs = result.get("matchedRefs") or result.get("policyRefs") or payload.get("policyRefs") or []
        if not isinstance(policy_refs, list):
            policy_refs = [str(policy_refs)]

        return cls(
            status=status,  # type: ignore[arg-type]
            reason=str(result.get("reason") or payload.get("reason") or "Allowed by Spctre policy."),
...
class EvidenceRecord(BaseModel):
    ...
    tool_parameters: Optional[Dict[str, Any]] = Field(default=None, alias="toolParameters")
