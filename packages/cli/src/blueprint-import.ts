...
function reportDryRun(
  envelope: { name: string; agentId: string },
  fileBase: string,
  branch: string,
  format: OutputFormat,
): void {
...
function reportSuccess(payload: ImportResponse, format: OutputFormat): void {
  ...
  console.log(
    `  policy:    branch ${payload.policyBranchId} @ revision ${payload.policyRevisionId}`,
  );
...
export async function blueprintImport(
  ...
  if (!file)
    fail("Error: a Blueprint source file path is required. Usage: spctre blueprint import <file>");
  ...
  const branch =
    typeof envelope.definition.policyBranchId === "string"
      ? envelope.definition.policyBranchId.trim()
      : "";
  ...
  if (!key) {
    fail(
      "Error: an operator API key is required. Pass --key (a blueprint:import service key) or run from a configured .spctre directory.",
    );
  }
