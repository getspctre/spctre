...
const EXPORT_FORMATS = new Set(["spctre-json", "opa-rego", "opa-bundle", "cedar", "mcp-proxy-config"]);
...
type BundleExportPreview = {
  formats?: PolicyBundleExportManifest[];
  error?: string;
};
...
async function resolveCliAuth(options: CliAuthOptions): Promise<{ workspace: string; key: string; url: string }> {
  let resolved = (!options.workspace || !options.key || !options.url)
    ? requireConfig()
    : readConfig();
...
export async function bundleExport(options: {
  ...
  try {
    ...
    const outputPath = path.resolve(process.cwd(), options.output ?? defaultArtifactPath(options.format));
    const manifestPath = path.resolve(process.cwd(), options.manifestOutput ?? `${outputPath}.manifest.json`);
...
export async function bundleVerify(options: {
  ...
  try {
    ...
    const manifest = JSON.parse(fs.readFileSync(manifestPath, "utf8")) as PolicyBundleExportManifest;
    ...
    if (options.output === "json") {
      console.log(JSON.stringify({
        ...verification,
        verificationTargets: manifest.verificationTargets,
      }, null, 2));
    } else {
  ...
  "spctre-json": [
    {
      ...
      type: "run",
      exe: "spctre",
      args: (artifact, manifest) => ["bundle", "verify", "--artifact", artifact, "--manifest", manifest],
    },
    {
...
export async function bundleRunVerifiers(options: {
  ...
  try {
    ...
    const formatMapped = Object.prototype.hasOwnProperty.call(FORMAT_VERIFIER_DISPATCH, manifest.format);
    ...
    const requiredTools = Array.from(
      new Set(specs.filter((s) => s.type === "run").map((s) => (s as Extract<VerifierSpec, { type: "run" }>).exe))
    );
    ...
    if (!formatMapped) {
      console.error(`No verifiers are configured for format "${manifest.format}"; nothing was checked.`);
    }

    if (options.output === "json") {
      console.log(JSON.stringify(
        { ok, passed: passCount, failed: failCount, skipped: skippedCount, informational: informationalCount, results },
        null,
        2
      ));
    } else {
      ...
      if (allRunnableSkipped) {
        const install = requiredTools.length > 0 ? requiredTools.join(" or ") : "the required verifier tools";
        console.error(`No verifier tools were found. Install ${install}, or pass --allow-missing-tools.`);
      }
    }
...
function readArtifactForManifest(artifactPath: string, manifest: PolicyBundleExportManifest): string | Record<string, unknown> {
  const raw = fs.readFileSync(artifactPath, "utf8");
  if (manifest.format === "spctre-json" || manifest.format === "opa-bundle" || manifest.format === "mcp-proxy-config") {
