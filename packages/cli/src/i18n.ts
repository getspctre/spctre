...
const catalogs: Record<SupportedLocale, Record<CliDiagnosticKey, string>> = {
  en: {
    ...
    "diagnostics.unknownAction": "action \"{action}\" not found for connector \"{connector}\" (known: {known})",
    "diagnostics.deadRule": "rule \"{ruleId}\" references only unknown connectors ({connectors}) - rule can never fire",
    "diagnostics.conflictShadow": "rule:{ruleId} shadowed by org baseline rule:{denyRuleId} (immutable, higher precedence)",
  ...
  ja: {
    ...
    "diagnostics.unknownConnector": "コネクタ \"{connector}\" はパックカタログにありません{suggestion}",
    "diagnostics.unknownAction": "アクション \"{action}\" はコネクタ \"{connector}\" にありません (既知: {known})",
    "diagnostics.deadRule": "ルール \"{ruleId}\" は不明なコネクタのみを参照しています ({connectors}) - このルールは発火しません",
    "diagnostics.conflictShadow": "rule:{ruleId} は組織ベースライン rule:{denyRuleId} によってシャドーされています (immutable, higher precedence)",
  ...
  de: {
    ...
    "diagnostics.unknownConnector": "Connector \"{connector}\" wurde im Pack-Katalog nicht gefunden{suggestion}",
    "diagnostics.unknownAction": "Aktion \"{action}\" wurde fuer Connector \"{connector}\" nicht gefunden (bekannt: {known})",
    "diagnostics.deadRule": "Regel \"{ruleId}\" verweist nur auf unbekannte Connectors ({connectors}) - Regel kann nie ausloesen",
    "diagnostics.conflictShadow": "rule:{ruleId} wird durch Organisations-Baseline rule:{denyRuleId} verdeckt (immutable, higher precedence)",
  ...
  fr: {
    ...
    "diagnostics.unknownConnector": "connecteur \"{connector}\" introuvable dans le catalogue de packs{suggestion}",
    "diagnostics.unknownAction": "action \"{action}\" introuvable pour le connecteur \"{connector}\" (connues : {known})",
    "diagnostics.deadRule": "la regle \"{ruleId}\" ne reference que des connecteurs inconnus ({connectors}) - elle ne peut jamais se declencher",
    "diagnostics.conflictShadow": "rule:{ruleId} est masquee par la regle de base organisationnelle rule:{denyRuleId} (immutable, higher precedence)",
