...
describe("bundle export", () => {
  beforeEach(() => {
    ...
    fs.writeFileSync(path.join(process.env.SPCTRE_CONFIG_DIR, "config.json"), JSON.stringify({
  ...
  it("previews all target export manifests without writing artifacts", async () => {
    const fetchSpy = vi.fn().mockResolvedValue({
      ...
      json: async () => ({
        formats: [
          ...
          { format: "cedar", blockingWarnings: ["Cedar cannot enforce ESCALATE semantics for rule r-1."] },
        ],
      }),
    });
    vi.stubGlobal("fetch", fetchSpy);
    ...
    expect(rows).toEqual([
      ...
      { format: "cedar", ok: false, blockingWarnings: ["Cedar cannot enforce ESCALATE semantics for rule r-1."] },
    ]);
  ...
  it("exits when the server blocks the target export", async () => {
    ...
    await bundleExport({
      format: "cedar",
      output: path.join(tmpDir, "policy.cedar"),
      quiet: true,
    });
  ...
  it("verifies a local artifact against its manifest", async () => {
    ...
    const result = await bundleVerify({
