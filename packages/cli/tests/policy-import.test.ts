...
describe("spctre policy import", () => {
  ...
  it("exits non-zero when the API returns an error", async () => {
    vi.stubGlobal(
      ...
      vi
        .fn()
        .mockResolvedValue(okResponse({ error: "Token is missing policy:import scope." }, 401)),
    );

    await expect(
      policyImport(policyFile, { key: "k", url: "https://control.test" }),
    ).rejects.toThrow("exit:1");
  });

  it("exits non-zero when no key is available", async () => {
    vi.stubGlobal("fetch", vi.fn());
    await expect(policyImport(policyFile, { url: "https://control.test" })).rejects.toThrow(
      "exit:1",
    );
  });
  ...
  it("exits non-zero when the file does not exist", async () => {
    vi.stubGlobal("fetch", vi.fn());
    await expect(policyImport(path.join(tmpDir, "missing.yaml"), { key: "k" })).rejects.toThrow(
      "exit:1",
    );
  });
});
