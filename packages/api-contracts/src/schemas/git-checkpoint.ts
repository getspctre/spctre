  ...
  checkpoint: z.object({
    ...
    diff: z.object({
      ...
      if (diff.format !== "none" && !diff.content && !diff.sha256 && !diff.files?.length) {
        ctx.addIssue({ code: "custom", message: "A diff must include content, sha256, or files." });
      }
      if (diff.format === "none" && (diff.content || diff.files?.length)) {
        ctx.addIssue({ code: "custom", message: "A diff with format 'none' cannot include content or files." });
      }
    }),
  }),
  agent: z.object({
