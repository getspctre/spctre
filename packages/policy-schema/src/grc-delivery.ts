...
export async function deliverGrcEvidenceBridge(params: {
  ...
  let url: URL;
  try { url = new URL(destination.endpoint); } catch { throw new Error("GRC destination endpoint must be an absolute URL."); }
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      const response = await params.send({ endpoint: destination.endpoint, body, headers: {
