...
describe("policy bundle export targets", () => {
  it("builds a provenance-bound Mastra runtime policy configuration", () => {
    ...
    expect(result.artifact).toMatchObject({
      schemaVersion: "spctre.mastra-policy.v1",
      provenance: { artifactHash: "sha256:artifact" },
    });
  });
  ...
  it("builds a provenance-bound Vercel AI runtime policy configuration", () => {
    ...
    expect(result.ok).toBe(true);
    expect(result.artifact).toMatchObject({
      schemaVersion: "spctre.vercel-ai-policy.v1",
      provenance: { revisionId: "revision-1" },
    });
  });

  it("builds a provenance-bound Genkit runtime policy configuration", () => {
    expect(buildGenkitRuntimePolicyConfig(makeBundle()).artifact).toMatchObject({
      schemaVersion: "spctre.genkit-policy.v1",
    });
  });

  it("builds a provenance-bound governance-sdk runtime policy configuration", () => {
    expect(buildGovernanceSdkRuntimePolicyConfig(makeBundle()).artifact).toMatchObject({
      schemaVersion: "spctre.governance-sdk-policy.v1",
    });
  });
  ...
  it("installs each TypeScript runtime middleware with reviewed provenance and payload enforcement", () => {
    for (const middleware of [
      ...
      expect(middleware.provenance).toMatchObject({
        artifactHash: "sha256:artifact",
        revisionId: "revision-1",
      });
  ...
  it("creates gateway-ingest evidence with payload hash but no payload content", () => {
    const evidence = buildGatewayPayloadEvidence({
      middleware: createMastraMiddleware(makeBundle()),
      decisionId: "decision-1",
      call: {
        connector: "github",
        action: "repo.delete",
        toolParameters: { repository: "private-repo" },
      },
    });
    expect(evidence).toMatchObject({
      ingestMode: "gateway",
      status: "DENY",
      policyRefs: ["github.repo.block_delete"],
    });
    expect(evidence.rawEvidence).toMatchObject({
      _payload_guardrail: true,
  ...
  it("binds Mastra hooks and tool executors to reviewed policy decisions", async () => {
    ...
    const mastra = createMastraBeforeToolCallHook({
      middleware: createMastraMiddleware(bundle),
      connector: "github",
      denyOutput: (result) => ({ blocked: result.reason }),
    });
    expect(mastra({ toolName: "repo.delete", input: {} })).toMatchObject({
      proceed: false,
      output: { blocked: expect.any(String) },
    });
    const wrappers = [
      wrapVercelAiToolExecute({
        middleware: createVercelAiMiddleware(bundle),
        connector: "github",
        toolName: "repo.delete",
        execute: async () => "ran",
      }),
      wrapGenkitToolExecute({
        middleware: createGenkitMiddleware(bundle),
        connector: "github",
        toolName: "repo.delete",
        execute: async () => "ran",
      }),
      wrapGovernanceSdkToolExecute({
        middleware: createGovernanceSdkMiddleware(bundle),
        connector: "github",
        toolName: "repo.delete",
        execute: async () => "ran",
      }),
    ];
    for (const execute of wrappers)
      await expect(execute({}, {})).rejects.toBeInstanceOf(SpctreToolDeniedError);
  });
  ...
  it("builds an idempotent webhook GRC delivery contract", () => {
    ...
    const packet = buildComplianceEvidenceExport({
      ...
      timeline: { branchId: "branch-1", revisionId: "revision-1", events: [] },
      evidence: [],
      generatedAt: "2026-07-05T00:00:00.000Z",
      retentionDays: 30,
    });
    expect(
      buildGrcEvidenceBridgeDelivery({
        packet,
        destination: { kind: "webhook", endpoint: "https://grc.example.test/evidence" },
      }),
    ).toMatchObject({
  ...
  it("retries transient GRC delivery failures with the same idempotency key", async () => {
    ...
    const delivery = buildGrcEvidenceBridgeDelivery({
      packet,
      destination: { kind: "webhook", endpoint: "https://grc.example.test/evidence" },
    });
    let calls = 0;
    const result = await deliverGrcEvidenceBridge({
      delivery,
      send: async ({ headers }) => {
        calls++;
        expect(headers["idempotency-key"]).toBe(delivery.idempotencyKey);
    ...
    const rules = [
      {
        ...baseRules[0],
        controlMappings: [
          { framework: "SOC2" as const, controlId: "CC6.1" },
          { framework: "SOC2" as const, controlId: "CC6.1" },
        ],
      },
    ];
    expect(validatePolicyControlMappings(rules)).toEqual([
      {
        stableRuleId: "github.repo.block_delete",
        message: "Duplicate control mapping SOC2:CC6.1.",
      },
    ]);
  });
  ...
  it("builds deterministic OPA bundle artifacts with data and Rego policy", () => {
    ...
    expect((first.artifact as Record<string, string>)["policy.rego"]).toContain(
  ...
  it("blocks exports that contain secret-like material", () => {
    const bundle = makeBundle({ metadata: { apiToken: "should-not-export" } });
  ...
  it("builds multiple requested target exports", () => {
    ...
    expect(exports.map((entry) => entry.format)).toEqual([
      "spctre-json",
      "opa-rego",
      "mcp-proxy-config",
    ]);
  ...
  it("verifies an export artifact against its manifest hash and provenance", () => {
    const exported = buildPolicyBundleExport({ bundle: makeBundle(), format: "opa-rego" });
