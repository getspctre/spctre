...
const rules: PolicyRuleSummary[] = [
  {
    ...
    immutable: false,
    controlMappings: [{ framework: "SOC2", controlId: "CC6.1", rationale: "Reviews high-value refunds." }],
  },
  {
    ...
    immutable: true,
    controlMappings: [{ framework: "SOC2", controlId: "CC6.1", rationale: "Prevents fund redirection." }],
  },
  {
...
describe("buildControlEvidenceRollup", () => {
  it("returns zero-count entries for controls with no matching evidence", () => {
    ...
    expect(rollup[0]).toMatchObject({ framework: "SOC2", controlId: "CC6.1", decisionCount: 0, deniedCount: 0, warnedCount: 0 });
    expect(rollup[0].stableRuleIds.sort()).toEqual(["stripe.payout.destination_change.block", "stripe.refund.high_value_review"]);
  });
  ...
  it("folds evidence into per-control decision/deny/warn counts and tracks the latest timestamp", () => {
    const rollup = buildControlEvidenceRollup({
      rules,
      evidence: [
        { policyRefs: ["stripe.refund.high_value_review"], status: "ESCALATE", createdAt: "2026-07-01T00:00:00Z" },
        { policyRefs: ["stripe.payout.destination_change.block"], status: "DENY", createdAt: "2026-07-10T00:00:00Z" },
        { policyRefs: ["stripe.dispute.evidence_submission.warn"], status: "WARN", createdAt: "2026-07-15T00:00:00Z" },
      ],
    });
    expect(rollup).toHaveLength(1);
