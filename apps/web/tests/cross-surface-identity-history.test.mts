...
const events: CrossSurfaceIdentityEvent[] = [
  { kind: "DECISION", at: "2026-07-19T10:00:00.000Z", surfaceAgentId: "agent-canonical", summary: "ALLOW on stripe.refund", status: "ALLOW", connector: "stripe", action: "refund", ref: "dec-1" },
  { kind: "DECISION", at: "2026-07-19T09:00:00.000Z", surfaceAgentId: "agent-notion", surfaceType: "notion-worker", summary: "DENY on notion.export", status: "DENY", ref: "dec-2" },
  { kind: "TRUST", at: "2026-07-19T11:00:00.000Z", surfaceAgentId: "agent-notion", surfaceType: "notion-worker", summary: "Trust 0.82 (-0.05)", status: "EVIDENCE_INGEST", ref: "trust-1", detail: { trustScore: 0.82, delta: -0.05 } },
  { kind: "REVIEW", at: "2026-07-19T12:00:00.000Z", surfaceAgentId: "agent-canonical", summary: "Reviewer ABORT on stripe.refund", status: "ABORT", ref: "dec-1" },
  { kind: "IDENTITY", at: "2026-07-18T00:00:00.000Z", surfaceAgentId: "agent-canonical", summary: "surface linked", status: "SURFACE_LINKED", ref: "id-1" },
];

describe("buildCrossSurfaceIdentityHistory", () => {
  it("merges events newest-first and preserves surface provenance", () => {
    const history = buildCrossSurfaceIdentityHistory({ canonicalAgentId: "agent-canonical", surfaces, events });
    expect(history.events.map((event) => event.ref)).toEqual(["dec-1", "trust-1", "dec-1", "dec-2", "id-1"]);
    ...
    expect(history.events.find((event) => event.ref === "dec-2")?.surfaceType).toBe("notion-worker");
  });
  ...
  it("applies the limit after sorting so the newest events survive", () => {
    const history = buildCrossSurfaceIdentityHistory({ canonicalAgentId: "agent-canonical", surfaces, events, limit: 2 });
