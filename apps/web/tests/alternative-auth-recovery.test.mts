...
const { linkSocialIdentity, unlinkSocialIdentity, listLinkedSocialIdentities } = await import("../lib/repositories/auth/principal");
const { generateRecoveryCodes, consumeRecoveryCode } = await import("../lib/repositories/auth/recovery");
const { listPrincipalSessions, revokeSessionAndRecord } = await import("../lib/repositories/auth/session");
...
describe.skipIf(!databaseAvailable)("Alternative Auth & Account Recovery repository contracts", () => {
  it("links, lists, and unlinks a social identity without affecting another tenant", async () => {
    ...
    await expect(listLinkedSocialIdentities({
      principalId: fixture.principalId,
      tenantId: fixture.tenantId,
  ...
  it("lists active sessions and records both durable revocation events", async () => {
    ...
    await expect(runWithTenantContext(fixture.tenantId, () => listPrincipalSessions({ tenantId: fixture.tenantId, principalId: fixture.principalId })))
      .resolves.toMatchObject([{ id: activeSessionId, user_agent: "Test browser", ip_address: "127.0.0.1" }]);

    await runWithTenantContext(fixture.tenantId, () => revokeSessionAndRecord({
    ...
    await expect(runWithTenantContext(fixture.tenantId, () => listPrincipalSessions({ tenantId: fixture.tenantId, principalId: fixture.principalId })))
      .resolves.toEqual([]);
    await expect(rawSql<{ event_type: string; session_id: string; ip_address: string; user_agent: string }[]>`
    ...
    `).resolves.toEqual([{ event_type: "SESSION_REVOKED", session_id: activeSessionId, ip_address: "127.0.0.1", user_agent: "Test browser" }]);
    ...
    `).resolves.toEqual([{ event_type: "IDENTITY_CHANGE", source_id: activeSessionId, source_table: "app_session" }]);
  });
});
