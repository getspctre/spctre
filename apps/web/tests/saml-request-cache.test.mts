...
vi.mock("@/lib/db", () => ({ sql: sqlSpy, rawSql: vi.fn((strings: any) => strings) }));

vi.mock("@spctre/platform/metrics", () => ({ registerDbPoolMetrics: vi.fn() }));
...
describe("SAML AuthnRequest cache repository", () => {
  ...
  describe("saveSamlAuthnRequestId", () => {
    ...
    it("rejects without issuing SQL when requestId is empty", async () => {
      await expect(
        saveSamlAuthnRequestId({ tenantId: "t", requestId: "", value: "v", ttlSeconds: 3600 }),
      ).rejects.toThrow("Failed to persist SAML AuthnRequest ID");
  ...
  it("releases only its own unconsumed lease", async () => {
    await releaseSamlAuthnRequestLease({
      tenantId: "tenant-a",
      requestId: "req-1",
      leaseId: "lease-1",
    });
