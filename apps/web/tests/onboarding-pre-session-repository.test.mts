...
const { exchangeCliOnboardingCode, startCliOnboarding } =
  await import("../lib/repositories/onboarding/cli");

describe("CLI onboarding pre-session persistence", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    rawSqlMock.mockResolvedValue([]);
    sqlMock.begin.mockImplementation(async (work: (tx: typeof txMock) => Promise<unknown>) =>
      work(txMock),
    );
  ...
  it("looks up the opaque exchange code through rawSql, then binds issuance to its approved tenant", async () => {
    ...
    expect(runWithTenantContextMock.mock.calls.every(([tenantId]) => tenantId === "tenant-1")).toBe(
      true,
    );
  });
});
