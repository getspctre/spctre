...
vi.mock("@/lib/service-tokens", () => ({
  authenticateServiceToken: vi.fn().mockResolvedValue({
    ok: true,
    auth: { tenantId: "demo-tenant", workspaceId: "demo-ws", principalId: "svc-test", scopes: ["evidence:write"] },
  }),
  hasBearerToken: () => true,
}));
...
vi.mock("@/lib/domains/compliance/service", () => ({
  ...
  getCompliancePacket: vi.fn().mockResolvedValue({
    export: {
...
vi.mock("@/lib/repositories/mfa", async (importOriginal) => {
  ...
  return {
    ...real,
    listRevocationHistory: vi.fn().mockResolvedValue([
      {
        tokenId: "tok-1",
        tenantId: "demo-tenant",
        workspaceId: "demo-ws",
...
vi.mock("@/lib/repositories/operations-log", async (importOriginal) => {
  ...
  return {
    ...real,
    getOperationsLogEventCounts: vi.fn().mockResolvedValue({
      EVIDENCE_INGEST: 50,
      POLICY_PUBLISH: 1,
      TOKEN_ISSUED: 3,
      TOKEN_REVOKED: 1,
...
vi.mock("@/lib/repositories/verification", async (importOriginal) => {
  ...
  return {
    ...real,
    getLatestVerificationStatus: vi.fn().mockResolvedValue(null),
  };
});
...
describe("GET /api/compliance/export — framework annotation", () => {
  ...
  it("sets frameworkAnnotation to null for unknown framework value", async () => {
    const req = new Request("http://localhost:3000/api/compliance/export?framework=not-a-real-framework");
    const res = await complianceExportGet(req);
    expect(res.status).toBe(200);
