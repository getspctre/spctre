...
const { getLatestManagedSimulationRegression, persistSimulationRun } = await import("../lib/repositories/evidence/simulation");
...
describe.skipIf(!databaseAvailable)("simulation run persistence repository contract", () => {
  it("round-trips retained-log regression metadata and replay findings as JSONB", async () => {
    ...
    const run = buildSimulationRun({
      ...
      results: [{ eventId: "decision-1", connector: "github", action: "repo.read", previousStatus: "ALLOW", proposedStatus: "ALLOW", delta: "UNCHANGED", matchedPolicyRefs: [], reason: "Allowed." }],
      regressionSummary: { coverage: "RETAINED_LOG", newlyDeniedExpectedWorkCount: 0, removedEscalationCoverageCount: 0, newlyAllowedHighRiskCount: 0, blockingCount: 0 },
    });

    const runId = await runWithTenantContext(fixture.tenantId, () => persistSimulationRun(run, fixture.workspaceId, fixture.tenantId));
    expect(runId).toMatch(/^[0-9a-f-]{36}$/i);
    await expect(runWithTenantContext(fixture.tenantId, () => getLatestManagedSimulationRegression(fixture)))
