...
import {
  buildAgentBlueprintRuntimeArtifact,
  diffAgentBlueprintRevisions,
...
describe("agent blueprint definition validation", () => {
  ...
  it("rejects incomplete targets and negative budgets", () => {
    expect(
      parseAgentBlueprintDefinition({ ...validDefinition, runtimeTargets: ["OPENAI_AGENTS"] })
        .error,
    ).toContain("include a stack string");
    expect(
      parseAgentBlueprintDefinition({ ...validDefinition, budgets: { maxCostUsdPerSession: -1 } })
        .error,
  ...
  it("compiles a published revision into a portable runtime artifact", () => {
    ...
    expect(artifact).toMatchObject({
      ...
      blueprint: {
        revisionId: "blueprint-revision-1",
        definitionHash: "sha256:blueprint-definition",
      },
  ...
  it("diffs only the declared operating-envelope fields that changed", () => {
    ...
    const compare = {
      ...
      definition: {
        ...validDefinition,
        tools: ["stripe.refunds.create", "stripe.refunds.cancel"],
        budgets: { maxTokensPerTurn: 6000 },
      },
    };
