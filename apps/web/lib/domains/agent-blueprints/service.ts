...
import {
  getAgentBlueprintApprovals as getAgentBlueprintApprovalsInTenant,
  upsertAgentBlueprintApproval,
...
export async function getPublishedAgentBlueprintRuntimeByAgent(
  ...
  return runWithTenantContext(params.tenantId, () =>
    getPublishedAgentBlueprintRuntimeByAgentInTenant(params),
  );
}
...
function runtimeTargets(value: unknown, issues: string[]): RuntimeTarget[] {
  ...
  for (const target of value) {
    if (
      ...
      typeof (target as Record<string, unknown>).stack !== "string"
    ) {
    ...
    targets.push({
      ...
      ...(typeof record.inferenceProvider === "string"
...
export function parseAgentBlueprintDefinition(value: unknown): {
  ...
  if (record.approvalPath !== undefined)
    definition.approvalPath = normalizedStrings(record.approvalPath, "approvalPath", issues);
  if (typeof record.policyBranchId === "string" && record.policyBranchId.trim())
    definition.policyBranchId = record.policyBranchId.trim();
  if (typeof record.policyRevisionId === "string" && record.policyRevisionId.trim())
    definition.policyRevisionId = record.policyRevisionId.trim();
...
export async function importBlueprintForToken(input: {
  ...
  if (!parsed.definition)
    return { error: parsed.error ?? "Invalid Blueprint definition.", status: 400 };
  const definition = parsed.definition;

  const branchName = definition.policyBranchId?.trim();
  if (!branchName) {
    return {
      error: "definition.policyBranchId (the connector policy branch name) is required for import.",
      status: 400,
    };
  }
  ...
  try {
    return await runWithTenantContext(
      input.tenantId,
      async (): Promise<ImportBlueprintForTokenResult> => {
        ...
        if (!existing) {
          ...
          if (!existing) {
            return {
              error: "Blueprint could not be created; its name may already be in use.",
              status: 409,
            };
          }
        }
...
export async function publishBlueprintRevision(input: {
  ...
  return runWithTenantContext(input.tenantId, async () => {
    const [workflow, approvals] = await Promise.all([
      ...
      getAgentBlueprintApprovalsInTenant({
        tenantId: input.tenantId,
    ...
    if (readiness.status !== "READY")
      return { error: readiness.blockingReasons.map((reason) => reason.message).join(" ") };
    const revision = await setAgentBlueprintRevisionStatusInTenant({
      ...input,
      status: "PUBLISHED",
    });
    return revision
      ? { revision }
      : { error: "Blueprint is not ready to publish; verify its linked policy is published." };
  });
}
...
export function getAgentBlueprintWorkspaceScope(
  params: Parameters<typeof getAgentBlueprintWorkspaceScopeInTenant>[0],
) {
  return runWithTenantContext(params.tenantId, () =>
    getAgentBlueprintWorkspaceScopeInTenant(params),
  );
}
export function getPublishedAgentBlueprintRuntime(
  params: Parameters<typeof getPublishedAgentBlueprintRuntimeInTenant>[0],
) {
  return runWithTenantContext(params.tenantId, () =>
    getPublishedAgentBlueprintRuntimeInTenant(params),
  );
}
...
export function createAgentBlueprintRevision(
  params: Parameters<typeof createAgentBlueprintRevisionInTenant>[0],
) {
  return runWithTenantContext(params.tenantId, () => createAgentBlueprintRevisionInTenant(params));
}
export function setAgentBlueprintRevisionStatus(
  params: Parameters<typeof setAgentBlueprintRevisionStatusInTenant>[0],
) {
  return runWithTenantContext(params.tenantId, () =>
    setAgentBlueprintRevisionStatusInTenant(params),
  );
}
export function getAgentBlueprintApprovals(
  params: Parameters<typeof getAgentBlueprintApprovalsInTenant>[0],
) {
  return runWithTenantContext(params.tenantId, () => getAgentBlueprintApprovalsInTenant(params));
}
export function rollbackAgentBlueprint(
  params: Parameters<typeof rollbackAgentBlueprintInTenant>[0],
) {
  return runWithTenantContext(params.tenantId, () => rollbackAgentBlueprintInTenant(params));
}
export function simulateAgentBlueprintRevision(
  params: Parameters<typeof simulateAgentBlueprintRevisionInTenant>[0],
) {
  return runWithTenantContext(params.tenantId, () =>
