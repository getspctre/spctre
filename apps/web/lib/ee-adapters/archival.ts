...
export interface ArchivalService {
  ...
  verifyLedgerChain?(tenantId: string, workspaceId: string): Promise<{ ok: boolean; error?: string; verifiedCount: number }>;
}
...
const fallbackService: ArchivalService = {
  async store() {
    throw new Error("Forensic archival is a premium Cloud feature and is not available on your plan.");
  },
  async retrieve() {
    return null;
  },
