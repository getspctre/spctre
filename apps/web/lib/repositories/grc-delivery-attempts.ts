...
export async function recordGrcDeliveryAttempt(params: {
  tenantId: string; workspaceId: string; destinationId: string; idempotencyKey: string; artifactHash: string;
  status: "DELIVERED" | "RETRYABLE_FAILURE" | "TERMINAL_FAILURE"; httpStatus?: number; errorCode?: string;
}) {
  if (!sql) return;
...
export async function listGrcDeliveryAttempts(params: { tenantId: string; workspaceId: string; destinationId?: string; limit?: number }) {
  if (!sql) return [];
  const limit = Math.max(1, Math.min(params.limit ?? 50, 200));
  return sql<{ id: string; destination_id: string; idempotency_key: string; artifact_hash: string; status: string; http_status: number | null; error_code: string | null; created_at: Date }[]>`
