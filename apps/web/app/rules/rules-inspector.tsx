...
function SortIndicator({
  ...
  if (col !== sortKey)
    return <ChevronDownIcon size={12} className="rulesSortIcon rulesSortIconMuted" />;
  return sortDir === "asc" ? (
...
export function RulesTable({ rules }: RulesTableProps) {
  ...
  const sorted = [...rules].sort((a, b) => {
    ...
    if (sortKey === "title") {
      ...
      av = a.effect ?? "";
      bv = b.effect ?? "";
    } else if (sortKey === "connectors") {
      av = (a.connectors ?? []).join(",");
      bv = (b.connectors ?? []).join(",");
    } else if (sortKey === "actions") {
      av = (a.actions ?? []).join(",");
      bv = (b.actions ?? []).join(",");
    } else if (sortKey === "sourceFormat") {
      av = a.sourceFormat ?? "";
      bv = b.sourceFormat ?? "";
    }
    ...
    <>
      <div className="auditTableWrapper rulesTableWrapper">
        <table className="auditTable rulesCatalogTable">
          <thead>
            <tr>
              ...
              >
                <span>
                  {t("table.rule_title")}{" "}
                  <SortIndicator col="title" sortKey={sortKey} sortDir={sortDir} />
                </span>
              </th>
              <th
                className="rulesSortableHeader"
                ...
                aria-sort={
                  sortKey === "effect" ? (sortDir === "asc" ? "ascending" : "descending") : "none"
                }
              >
                <span>
                  {t("table.effect")}{" "}
                  <SortIndicator col="effect" sortKey={sortKey} sortDir={sortDir} />
                </span>
              </th>
              <th
                className="rulesSortableHeader"
              ...
              >
                <span>
                  {t("table.connector")}{" "}
                  <SortIndicator col="connectors" sortKey={sortKey} sortDir={sortDir} />
                </span>
              </th>
              <th
                className="rulesSortableHeader"
              ...
              >
                <span>
                  {t("table.actions")}{" "}
                  <SortIndicator col="actions" sortKey={sortKey} sortDir={sortDir} />
                </span>
              </th>
              <th
                className="rulesSortableHeader"
              ...
              >
                <span>
                  {t("table.source")}{" "}
                  <SortIndicator col="sourceFormat" sortKey={sortKey} sortDir={sortDir} />
                </span>
              </th>
              <th>
          ...
          <tbody>
            {sorted.map((rule) => (
              <tr key={rule.stableRuleId} className="auditRow">
                <td>
                  <strong>{rule.title}</strong>
                  {rule.immutable ? (
                    <span className="pill ruleInlinePill">{t("table.immutable")}</span>
                  ) : null}
                </td>
                <td>
      ...
      {selected ? <RuleInspectorPanel rule={selected} onClose={() => setSelected(null)} /> : null}
    </>
  );
}
      ...
      <div className="packRuleMeta" style={{ display: "flex", flexDirection: "column", gap: 8 }}>
        {rule.semanticChecks.map((check) => (
          <div key={check.id} className="ruleConditionBlock" style={{ padding: "8px 12px" }}>
            ...
            <div className="meta" style={{ color: "var(--foreground)" }}>
              {check.prompt}
            </div>
            {check.effect ? (
...
function RuleInspectorPanel({ rule, onClose }: PanelProps) {
  ...
  return (
    <Drawer
      ...
      width="wide"
      eyebrow={t("panel.managed_rule")}
      title={rule.title}
      description={rule.stableRuleId}
    >
