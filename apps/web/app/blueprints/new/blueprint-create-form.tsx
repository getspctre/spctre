...
function ChipInput({
  ...
  return (
    <div className="field blueprintChipField">
      <span>{label}</span>
      <div className="blueprintChipControl">
        {values.length ? (
          <div className="blueprintChipList">
            {values.map((value) => (
              <span
                className={`blueprintChip${custom(value) ? " blueprintChipCustom" : ""}`}
                key={value}
              >
                {value}
                <button
...
export function BlueprintCreateForm({
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
        ...
        runtimeTargets: [
          {
            ...
            ...(String(data.get("adapter") ?? "").trim()
              ? { adapter: String(data.get("adapter")).trim() }
              : {}),
          },
        ],
        ...(Object.keys(budgets).length ? { budgets } : {}),
        ...(list(String(data.get("approvalPath") ?? "")).length
          ? { approvalPath: list(String(data.get("approvalPath") ?? "")) }
          : {}),
        ...(String(data.get("policyBranchId") ?? "").trim()
          ? { policyBranchId: String(data.get("policyBranchId")).trim() }
          : {}),
        ...(String(data.get("policyRevisionId") ?? "").trim()
          ? { policyRevisionId: String(data.get("policyRevisionId")).trim() }
          : {}),
      },
    };
    ...
    setPending(true);
    try {
      const response = await fetch("/api/agent-blueprints", {
        method: "POST",
  ...
  return (
    <form className="blueprintCreateForm" onSubmit={submit}>
      <section className="panel blueprintCreatePanel">
        <div className="rowHeader">
          <div>
            <p className="eyebrow">Identity</p>
            <h2>Describe the governed agent</h2>
            <p className="meta">
              This draft defines the agent’s permitted operating envelope for {workspaceName}.
            </p>
          </div>
        </div>
        <div className="blueprintFormGrid">
          <label className="field">
            ...
            <input className="input" name="name" placeholder="Refund support agent" required />
          </label>
          <label className="field">
            <span>Agent ID</span>
            <input className="input" name="agentId" placeholder="support-refunds" required />
          </label>
          <label className="field blueprintFormWide">
            <span>Purpose</span>
            <textarea
              className="input"
              name="purpose"
              rows={3}
              placeholder="Resolve customer refund requests within the approved support workflow."
              required
            />
          </label>
          <label className="field blueprintFormWide">
            ...
            <input
              className="input"
              name="message"
              placeholder="Initial support operating envelope"
            />
          </label>
        </div>
      </section>
      ...
      <section className="panel blueprintCreatePanel">
        <div className="rowHeader">
          <div>
            ...
            <h2>Declare permitted surfaces</h2>
            <p className="meta">
              Select a catalog value or enter a deliberate custom declaration. Only declared
              connectors and tools may run after publication.
            </p>
          </div>
        </div>
        <div className="blueprintFormGrid">
          ...
          <ChipInput
            label="Tools"
            name="tools"
            placeholder="Search known actions"
          ...
          <fieldset className="blueprintEnvironmentField blueprintFormWide">
            <legend>Environments</legend>
            <div className="checkboxGroup">
              {ENVIRONMENTS.map((environment) => (
                <label className="checkboxLabel" key={environment}>
                  <input
                    name="environments"
                    type="checkbox"
                    value={environment}
      ...
      <section className="panel blueprintCreatePanel">
        <div className="rowHeader">
          <div>
            <p className="eyebrow">Runtime and review</p>
            <h2>Bind execution and evidence</h2>
            <p className="meta">
              The draft starts unapproved. A linked policy revision is enforced only once both
              policy and Blueprint are publishable.
            </p>
          </div>
        </div>
        <div className="blueprintFormGrid">
          <label className="field">
            ...
            <select className="input" name="runtimeStack" defaultValue="OPENAI_AGENTS">
          ...
          <label className="field">
            ...
            <select className="input" name="runtimeEnvironment" defaultValue="production">
              {ENVIRONMENTS.map((environment) => (
          ...
          <label className="field">
            <span>Adapter, optional</span>
            <input className="input" name="adapter" placeholder="gateway-v2" />
          </label>
          <label className="field">
            <span>Approval path, optional</span>
            <input className="input" name="approvalPath" placeholder="Support, Finance" />
          </label>
          <label className="field">
            <span>Policy branch ID, optional</span>
            <input className="input" name="policyBranchId" placeholder="branch_…" />
          </label>
          <label className="field">
            <span>Policy revision ID, optional</span>
            <input className="input" name="policyRevisionId" placeholder="revision_…" />
          </label>
        </div>
        <div className="blueprintBudgetGrid" aria-label="Optional session budgets">
          ...
          <label className="field">
            <span>Max session cost, USD</span>
            <input
              className="input"
              min="0"
              name="maxCostUsdPerSession"
              placeholder="2"
              step="0.01"
              type="number"
            />
          ...
          <label className="field">
            <span>Max tool calls per session</span>
            <input
              className="input"
              min="0"
              name="maxToolCallsPerSession"
              placeholder="12"
              type="number"
            />
          </label>
      ...
      {error ? (
        <p className="blueprintCreateError" role="alert">
          {error}
        </p>
      ) : null}
      <div className="blueprintCreateActions">
