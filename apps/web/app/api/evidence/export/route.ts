...
async function handleGetApiEvidenceExport(request: Request) {
  ...
  try {
    ...
    incrementCounter("spctre.api.errors", 1, { "http.route": "/api/evidence/export", "http.response.status_code": 503 });
    console.error("[evidence/export] getActiveScope failed", err);
    return withTraceId(Response.json({ error: "Workspace context unavailable.", meta: makeMeta(traceId) }, { status: 503 }), traceId);
  }

  if (!workspaceContext.workspaceId || !workspaceContext.tenantId) {
    incrementCounter("spctre.api.errors", 1, { "http.route": "/api/evidence/export", "http.response.status_code": 400 });
  ...
  if (format === "agt-verification") {
    ...
    if (!exportInputs) {
      incrementCounter("spctre.api.errors", 1, { "http.route": "/api/evidence/export", "http.response.status_code": 404 });
      return withTraceId(Response.json(
        { error: "No published policy bundle is available for AGT verification export.", meta: makeMeta(traceId) },
        { status: 404 }
      ), traceId);
    }

    const { published, escalations, verificationResults } = exportInputs;
    const scopedEvidence = evidence.filter((record) =>
      record.artifactHash === published.artifactHash ||
      record.policyContext.some((context) =>
  ...
  recordDuration("spctre.evidence.export.duration", Date.now() - started, { format });
  return withTraceId(new Response(header + rows, {
    headers: {
