...
async function handleGetApiForensicEvidence(request: Request) {
  ...
  if (!isFeatureEnabledForPlan("longTermForensicArchival", plan)) {
    return withTraceId(
      Response.json({ error: "Forensic evidence query requires a Cloud or Enterprise plan.", meta: makeMeta(traceId) }, { status: 402 }),
      traceId
    );
  }
  ...
  const limit = Math.min(Math.max(Number(url.searchParams.get("limit") ?? "1000") || 1000, 1), MAX_LIMIT);
  ...
  try {
    ...
    if (err instanceof Error && err.message === "Database not configured.") {
      return withTraceId(
        Response.json({ error: "Database not configured.", meta: makeMeta(traceId) }, { status: 503 }),
        traceId
      );
    }
    throw err;
  ...
  const nextCursor = rows.length > limit ? page[page.length - 1]?.created_at.toISOString() ?? null : null;
  ...
  return withTraceId(
    Response.json({
      records: page.map((row) => ({
        ...
        rawEvidence: row.raw_evidence && typeof row.raw_evidence === "object" ? row.raw_evidence : {},
