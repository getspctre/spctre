...
async function handlePostApiTrustIngest(request: Request) {
  ...
  if (!environment)
    return withTraceId(
      Response.json(
        { error: "environment is required.", meta: makeMeta(traceId) },
        { status: 400 },
      ),
      traceId,
    );
  if (!runtimeStack || !VALID_STACKS.has(runtimeStack)) {
    return withTraceId(
      Response.json(
        { error: "runtimeStack must be a supported runtime stack.", meta: makeMeta(traceId) },
        { status: 400 },
      ),
      traceId,
    );
  ...
  if (trustScore === undefined || trustScore < 0 || trustScore > 1) {
    return withTraceId(
      Response.json(
        { error: "trustScore must be a number between 0 and 1.", meta: makeMeta(traceId) },
        { status: 400 },
      ),
      traceId,
    );
  ...
  if (!source || !VALID_SOURCES.has(source)) {
    return withTraceId(
      Response.json(
        { error: "source must be a valid TrustScoreSource.", meta: makeMeta(traceId) },
        { status: 400 },
      ),
      traceId,
    );
  ...
  return withTraceId(
    Response.json({ ok: true, agentId, trustScore, meta: makeMeta(traceId) }, { status: 201 }),
    traceId,
  );
}
