...
import { ingestContextBudget, listContextBudget, recordTrustOperation } from "@/lib/domains/trust/service";
...
async function handleGetApiTrustContextBudget(request: Request) {
  ...
  if (eventType && !VALID_EVENT_TYPES.has(eventType)) {
    return withTraceId(Response.json({ error: "Invalid eventType.", meta: makeMeta(traceId) }, { status: 400 }), traceId);
  }

  let events;
  try {
    events = await listContextBudget({ tenantId: ctx.tenantId, workspaceId: ctx.workspaceId, sessionId, agentId, eventType, limit });
  } catch (err) {
...
function parseContextBudgetFields(rec: Record<string, unknown>): ContextBudgetFields | { error: string } {
  ...
  if (!eventType || !VALID_EVENT_TYPES.has(eventType)) {
    return { error: "eventType must be one of: TOKEN_GROWTH, SUMMARIZATION_EVENT, CONTEXT_SOURCE_MIX, BUDGET_BREACH." };
  }
...
async function handlePostApiTrustContextBudget(request: Request) {
  ...
  if ("error" in fields) {
    return withTraceId(Response.json({ error: fields.error, meta: makeMeta(traceId) }, { status: 400 }), traceId);
  }
  const { sessionId, agentId, environment, runtimeStack, eventType, tokenCount } = fields;

  const budgetLimit = asInt(rec.budgetLimit);
  const budgetUtilization = budgetLimit && budgetLimit > 0 ? tokenCount / budgetLimit : asNumber(rec.budgetUtilization);

  const delegated = await delegateTrustPostToWorker({ path: "context-budget", auth, body: rec, traceId });
  ...
  try {
    id = await ingestContextBudget({
      ...
      contextSourceMix: isRecord(rec.contextSourceMix) ? rec.contextSourceMix as Record<string, number> : undefined,
      budgetLimit,
      budgetUtilization,
      governanceAction: asString(rec.governanceAction) as "ALLOW" | "WARN" | "ESCALATE" | "REVIEW" | undefined,
