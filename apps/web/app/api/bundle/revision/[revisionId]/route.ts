...
export async function GET(
  ...
  if (!branchId) {
    return withTraceId(
      Response.json({ error: "branch is required.", meta: makeMeta(traceId) }, { status: 400 }),
      traceId,
    );
  }
  if (format && !EXPORT_FORMATS.has(format)) {
    return withTraceId(
      Response.json(
        { error: "Unsupported bundle export format.", meta: makeMeta(traceId) },
        { status: 400 },
      ),
      traceId,
    );
  ...
  const branch = await listBranches(scope.workspaceId, scope.tenantId)
    .then((branches) => branches.find((item) => item.id === branchId))
    .catch(swallow("listBranches", undefined));
  if (!branch || branch.activeRevision !== revisionId) {
    return withTraceId(
      Response.json(
        {
          error: "The reviewed revision is unavailable in this workspace.",
  ...
  if (!artifacts) {
    return withTraceId(
      Response.json(
        { error: "The reviewed revision is unavailable.", meta: makeMeta(traceId) },
        { status: 404 },
      ),
      traceId,
    );
  ...
  if (format) {
    const exported = buildPolicyBundleExport({
      ...
      format,
      generatedAt: artifacts.bundle.generatedAt,
    });
    ...
    const body = exported.ok
      ...
      : {
          error: "Bundle export is blocked because target semantics could not be preserved.",
          manifest: exported.manifest,
          meta: makeMeta(traceId),
        };
    return withTraceId(
  ...
  return withTraceId(
    new Response(JSON.stringify(artifacts.bundle, null, 2), {
      headers: {
