...
import { getVerifiedSmsEnrollment, isSmsCooldownActive, updateSmsEnrollmentSecret } from "@/lib/domains/auth/service";
...
async function handlePostApiAuthMfaSmsSendOtp(request: Request) {
  ...
  if (!enrollment) {
    const response = NextResponse.json({ error: "No verified SMS MFA enrollment found.", meta: makeMeta(traceId) }, { status: 404 });
    response.headers.set("x-request-id", traceId);
    return response;
  }
