...
export async function GET(request: Request) {
  ...
  const destinations = await listGrcDeliveryDestinations({
...
export async function POST(request: Request) {
  ...
  if (!write.allowed)
    return withTraceId(
      Response.json(
        { error: write.error ?? "Write access denied.", meta: makeMeta(auth.traceId) },
        { status: 403 },
      ),
      auth.traceId,
    );
  ...
  const kind =
    typeof body?.kind === "string" && KINDS.has(body.kind as GrcDeliveryDestination["kind"])
      ? (body.kind as GrcDeliveryDestination["kind"])
      : null;
  const endpoint = typeof body?.endpoint === "string" ? body.endpoint.trim() : "";
  const label = typeof body?.label === "string" ? body.label.trim() : "";
  const credential = typeof body?.credential === "string" ? body.credential : undefined;
  if (!kind || !endpoint || !label)
    return withTraceId(
      Response.json(
        { error: "kind, HTTPS endpoint, and label are required.", meta: makeMeta(auth.traceId) },
        { status: 400 },
      ),
      auth.traceId,
    );
  try {
    const id = await createGrcDeliveryDestination({
      tenantId: auth.scope.tenantId,
      workspaceId: auth.scope.workspaceId,
      kind,
      endpoint,
      label,
      credential,
      createdBy: auth.session.principalId,
    });
    appendOperationsLog({
      tenantId: auth.scope.tenantId,
      workspaceId: auth.scope.workspaceId,
      eventType: "GRC_DESTINATION_CONFIGURED",
      sourceId: id,
      sourceTable: "grc_delivery_destination",
      actorId: auth.session.principalId,
      payload: { action: "CREATED", kind, endpoint, label, hasCredential: Boolean(credential) },
    }).catch(swallow("appendOperationsLog", undefined));
    return withTraceId(
      Response.json({ id, meta: makeMeta(auth.traceId) }, { status: 201 }),
      auth.traceId,
    );
  } catch (error) {
    return withTraceId(
      Response.json(
        {
          error: error instanceof Error ? error.message : "Destination could not be created.",
