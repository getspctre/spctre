...
const handleGetGrcDeliveryAttempts = withApiRoute("/api/compliance/grc-destinations/attempts", async (request, ctx) => {
  ...
  const attempts = await listGrcDeliveryAttempts({ tenantId: scope.tenantId, workspaceId: scope.workspaceId, destinationId: url.searchParams.get("destinationId")?.trim() || undefined, limit });
  return ctx.json({ attempts: attempts.map((attempt) => ({ ...attempt, createdAt: attempt.created_at.toISOString() })) });
});
