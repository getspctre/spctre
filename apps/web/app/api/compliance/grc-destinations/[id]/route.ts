...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const [session, scope, route] = await Promise.all([
  ...
  const credential =
    typeof body?.credential === "string" && body.credential ? body.credential : undefined;
  if (enabled === undefined && !credential)
    return withTraceId(
      Response.json(
        { error: "enabled or credential is required.", meta: makeMeta(traceId) },
        { status: 400 },
      ),
      traceId,
    );
  const id = await updateGrcDeliveryDestination({
  ...
  if (!id)
    return withTraceId(
      Response.json({ error: "Destination not found.", meta: makeMeta(traceId) }, { status: 404 }),
      traceId,
    );
  appendOperationsLog({
    tenantId: scope.tenantId,
    ...
    payload: {
      action: credential ? "CREDENTIAL_ROTATED" : "ENABLED_CHANGED",
      enabled,
