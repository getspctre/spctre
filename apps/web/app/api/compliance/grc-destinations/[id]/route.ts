...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const [session, scope, route] = await Promise.all([getAuthSession().catch(swallow("getAuthSession", null)), getActiveScope().catch(swallow("getActiveScope", null)), params]);
  ...
  appendOperationsLog({ tenantId: scope.tenantId, workspaceId: scope.workspaceId, eventType: "GRC_DESTINATION_CONFIGURED", sourceId: id, sourceTable: "grc_delivery_destination", actorId: session.principalId, payload: { action: credential ? "CREDENTIAL_ROTATED" : "ENABLED_CHANGED", enabled, credentialRotated: Boolean(credential) } }).catch(swallow("appendOperationsLog", undefined));
