...
async function handleGetApiComplianceExport(request: Request) {
  ...
  return await withSpan(
    ...
    { "spctre.request_id": traceId, "http.route": "/api/compliance/export" },
    async (span) => {
      ...
      if (hasBearer) {
        ...
        if (!auth.ok) {
          incrementCounter("spctre.api.errors", 1, {
            "http.route": "/api/compliance/export",
      ...
      const frameworkAnnotation = framework
        ? buildComplianceFrameworkAnnotation({
            ...
            resolvedEscalationCount: packet.escalations.filter(
              (e) => e.resolutionOutcome !== "ESCALATE",
            ).length,
            artifactHash: packet.export.artifactHash,
      ...
      if (url.searchParams.get("format")?.toLowerCase() === "grc") {
        return withTraceId(
          Response.json(buildGrcEvidenceBridgeExport(packet.export), {
            headers: {
      ...
      recordComplianceExportConversion(tenantId).catch(
        swallow("recordComplianceExportConversion", undefined),
      );

      recordDuration("spctre.compliance.export.duration", Date.now() - started, {
