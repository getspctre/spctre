...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!result)
    return withTraceId(
      Response.json(
        { error: "Blueprint not found.", meta: makeMeta(auth.traceId) },
        { status: 404 },
      ),
      auth.traceId,
    );
  ...
  if (compareRevisionId) {
    ...
    const base = result.revisions.find(
      (revision) => revision.id === (baseRevisionId || compare?.parentRevisionId),
    );
    if (!compare || !base) {
      return withTraceId(
        Response.json(
          {
            error:
              "Both Blueprint revisions must exist; provide baseRevisionId when the compared revision has no parent.",
  ...
  return withTraceId(
    Response.json({ ...result, ...(diff ? { diff } : {}), meta: makeMeta(auth.traceId) }),
    auth.traceId,
  );
}
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!workspaceScope || workspaceScope.workspace_id !== auth.scope.workspaceId)
    return withTraceId(
      Response.json(
        { error: "Blueprint not found.", meta: makeMeta(auth.traceId) },
        { status: 404 },
  ...
  if (!parsed.definition)
    return withTraceId(
      Response.json({ error: parsed.error, meta: makeMeta(auth.traceId) }, { status: 400 }),
      auth.traceId,
    );
  ...
  if (!revision)
    return withTraceId(
      Response.json(
        {
          error: "Blueprint not found or this definition is already committed.",
          meta: makeMeta(auth.traceId),
        },
        { status: 409 },
      ),
  ...
  return withTraceId(
    Response.json({ revision, meta: makeMeta(auth.traceId) }, { status: 201 }),
    auth.traceId,
  );
}
...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!revisionId || !status || !["IN_REVIEW", "PUBLISHED"].includes(status))
    return withTraceId(
      Response.json(
        {
          error: "revisionId and a valid lifecycle transition are required.",
          meta: makeMeta(auth.traceId),
        },
        { status: 400 },
      ),
  ...
  if (status === "PUBLISHED") {
    const permissions = actor
      ? getBranchPermissions({
          ...
          workspaceSlug: workspaceScope.workspace_slug ?? "workspace-demo",
        })
    ...
    if (!permissions.canPublish)
      return withTraceId(
        Response.json(
          {
            error: permissions.publishReason ?? "Publish is not allowed.",
            meta: makeMeta(auth.traceId),
          },
          { status: 403 },
        ),
  ...
  const result =
    status === "PUBLISHED"
      ? await publishBlueprintRevision({
          tenantId: auth.scope.tenantId,
          workspaceId: auth.scope.workspaceId,
          blueprintId: id,
          revisionId,
      ...
      : {
          revision: await setAgentBlueprintRevisionStatus({
            tenantId: auth.scope.tenantId,
            workspaceId: auth.scope.workspaceId,
            blueprintId: id,
            revisionId,
  ...
  if ("error" in result)
    return withTraceId(
      Response.json({ error: result.error, meta: makeMeta(auth.traceId) }, { status: 409 }),
      auth.traceId,
    );
  ...
  if (!revision)
    return withTraceId(
      Response.json(
        {
          error: "Blueprint revision not found or not in the required lifecycle state.",
          meta: makeMeta(auth.traceId),
        },
        { status: 404 },
      ),
