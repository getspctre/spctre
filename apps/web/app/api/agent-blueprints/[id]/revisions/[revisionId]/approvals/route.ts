...
export async function GET(
  ...
  const blueprint = await getAgentBlueprint({
    tenantId: scope.tenantId,
    workspaceId: scope.workspaceId,
    blueprintId,
  });
  if (!blueprint || !blueprint.revisions.some((revision) => revision.id === revisionId))
    return withTraceId(
      Response.json(
...
export async function POST(
  ...
  if (
    !(ALL_REVIEWER_ROLES as readonly string[]).includes(role) ||
    !["APPROVED", "CHANGES_REQUESTED"].includes(String(status))
  )
    return withTraceId(
      Response.json(
        { error: "A valid reviewer role and status are required.", meta: makeMeta(traceId) },
        { status: 400 },
      ),
      traceId,
    );
  ...
  if (!actor)
    return withTraceId(
      Response.json(
        { error: "Reviewer principal is not available.", meta: makeMeta(traceId) },
        { status: 403 },
      ),
      traceId,
    );
  ...
  const review = canActorReviewRole(
    actor,
    workspaceScope?.workspace_slug ?? "workspace-demo",
    role,
  );
  if (!review.allowed)
    return withTraceId(
      Response.json(
        { error: review.reason ?? "Review is not allowed.", meta: makeMeta(traceId) },
        { status: 403 },
      ),
      traceId,
    );
  const ok = await submitBlueprintApproval({
    tenantId: scope.tenantId,
    workspaceId: scope.workspaceId,
    blueprintId,
    revisionId,
    reviewerId: actor.id,
    reviewerRole: role,
    status: status as "APPROVED" | "CHANGES_REQUESTED",
  ...
  appendOperationsLog({
    tenantId: scope.tenantId,
    workspaceId: scope.workspaceId,
    eventType: "BLUEPRINT_APPROVE",
    sourceId: revisionId,
    sourceTable: "agent_blueprint_approval",
    actorId: actor.id,
    payload: { blueprintId, role, status },
