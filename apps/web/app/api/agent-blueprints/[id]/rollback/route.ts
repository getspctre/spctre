...
import { rollbackAgentBlueprint } from "@/lib/domains/agent-blueprints/service";
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!actor || !requireActorAdminWorkspace(actor, "workspace-demo").allowed) return withTraceId(Response.json({ error: "Admin permission is required to roll back a Blueprint.", meta: makeMeta(traceId) }, { status: 403 }), traceId);
  const body = await request.json().catch(() => null) as Record<string, unknown> | null;
  const targetRevisionId = typeof body?.targetRevisionId === "string" ? body.targetRevisionId.trim() : "";
  if (!targetRevisionId) return withTraceId(Response.json({ error: "targetRevisionId is required.", meta: makeMeta(traceId) }, { status: 400 }), traceId);
