...
async function handlePostApiGatewayResolve(request: Request) {
  ...
  return await withSpan(
    "api.gateway.resolve",
    { "spctre.request_id": traceId, "http.route": "/api/gateway/resolve" },
    async () => {
      const session = await getAuthSession().catch(swallow("getAuthSession", null));
      if (!session) {
        incrementCounter("spctre.api.errors", 1, {
          "http.route": "/api/gateway/resolve",
      ...
      if (isDemoTenant(workspaceContext.tenantId)) {
        return withTraceId(
          Response.json(
            {
              error: "Escalation resolution is not available in Demo Mode.",
      ...
      if (!ok) {
        ...
        return withTraceId(
          Response.json(
            {
              error: "Escalation queue item not found or already resolved.",
              meta: makeMeta(traceId),
            },
            { status: 404 },
          ),
      ...
      recordDuration("spctre.gateway.resolve.duration", Date.now() - started, {
        outcome: parsed.value.resolutionOutcome,
      });
