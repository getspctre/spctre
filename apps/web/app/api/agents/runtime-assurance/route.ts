 ...
 */
const handleGetRuntimeAssurance = withApiRoute(
  "/api/agents/runtime-assurance",
  async (request, ctx) => {
    ...
    let inventory =
      status === "CURRENT" || status === "DRIFTED" || status === "PROVENANCE_GAP"
    ...
    return ctx.json({
      coverage: {
        ...
        alerts: model.runtimeCoverage.alerts.filter((alert) =>
          inventory.some(
            (runtime) =>
              runtime.agentId === alert.agentId && runtime.runtimeTarget === alert.runtimeTarget,
          ),
        ),
