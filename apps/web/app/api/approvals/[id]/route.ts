...
async function handleGetApiApprovalsByid(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const approval = await getApprovalDetail({ approvalId: id, workspaceId: ctx.workspaceId, tenantId: ctx.tenantId }).catch(swallow("getApprovalDetail", null));
  if (!approval) return withTraceId(Response.json({ error: "Approval not found.", meta: makeMeta(traceId) }, { status: 404 }), traceId);
