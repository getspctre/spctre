...
async function handlePostApiTokenRevoke(request: Request) {
  ...
  return await withSpan("api.token.revoke", spanAttrs, async () => {
    ...
    if (revokeResult === "db-unavailable") {
      ...
      return withTraceId(
        Response.json({ error: "Database not configured." }, { status: 503 }),
        traceId,
      );
    }
    ...
    recordDuration("spctre.token.revoke.duration", Date.now() - started, {
      "http.route": ROUTE_PATH,
    });
...
async function delegateTokenRevokeToWorker(
  request: Request,
  traceId: string,
