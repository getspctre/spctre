...
| `SPCTRE_PROVISIONING_SECRET`        | No                        | Shared secret accepted by `POST /api/internal/provisioning/tenant`, which creates the tenant, workspace, owner and baseline policy for a completed hosted checkout. Required only where a checkout surface provisions tenants; unset leaves the endpoint returning 500. This credential may only provision the plans in `SPCTRE_PROVISIONING_ALLOWED_PLANS` and its tenants are customers. |
| `SPCTRE_PROVISIONING_ALLOWED_PLANS` | No                        | Comma-separated plans `SPCTRE_PROVISIONING_SECRET` may provision. Defaults to `HOSTED_TRIAL,TEAM,BUSINESS` — a checkout surface has no reason to mint a tier that is only sold through an order form, so a leak of its secret cannot. Widen it if your own tooling drives provisioning; nothing here enforces a licence.                                                                   |
| `SPCTRE_PROVISIONING_GRANT_SECRET`  | No                        | Separate operator credential for internal accounts — employees, testers, dogfooding. May provision any plan, and its tenants are marked `INTERNAL`: their usage is measured and reported but never charged, because no subscription exists behind them. Unset means the endpoint offers no way to create one.                                                                              |
