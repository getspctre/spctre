-- Gateway decisions are first-write-wins audit records: a replay resolves to
-- the original row and never rewrites it. A replay whose evaluation disagrees
-- with the persisted decision must still be inspectable, so the worker records
-- it as its own operations-log event rather than discarding it.
